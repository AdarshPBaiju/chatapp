...
from core.validators import RequiredRule, EmailFormatRule, MinMaxLengthRule, ServiceValidator
...
class ValidatorTests(TestCase):
    ...
    def test_email_format_rule(self):
        ...
        # Should fail
        with self.assertRaises(ValidationError):
            rule.validate("email", "invalid-email", {})

    def test_min_max_length_rule(self):
        rule = MinMaxLengthRule(min_len=3, max_len=5)
        # Should pass
        rule.validate("name", "abc", {})
        rule.validate("name", "abcde", {})
        # Should fail min
        with self.assertRaises(ValidationError):
            rule.validate("name", "ab", {})
        # Should fail max
        with self.assertRaises(ValidationError):
            rule.validate("name", "abcdef", {})
