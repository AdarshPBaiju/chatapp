...
def _build_restricted_payload(*, user_id: str, request, context, session_id: str | None):
    ...
    return {
        ...
        "active_sessions": restricted_tokens["active_sessions"],
    }
...
class ClientSessionListAPIView(APIView):
    ...
    def get(self, request):
        ...
        # list_active_sessions needs to be migrated to SessionQueryService
        ...
        if request.auth.get("scope") == "revoke_only":
            try:
                res = TokenRotateService.promote_restricted_session(
                    user_id=user_id,
                    access_jti=request.auth["jti"],
                    refresh_jti=request.auth["partner_jti"],
...
class ClientSessionRevokeAPIView(APIView):
    ...
    def post(self, request):
        ...
        context = build_auth_request_context(request)
        current_session = SessionQueryService.get_active_session(
            user_id=user_id,
            session_id=request.auth.get("sid"),
            access_jti=request.auth.get("jti"),
            # Note: adjust context fields if needed
        )
        ...
        revoked = SessionManager.revoke_session(
            user_id=user_id,
...
class ClientSessionPromoteAPIView(APIView):
    permission_classes = [AllowRevokeOnly]

    @extend_schema(tags=["Client Security"])
    def post(self, request):
        """
        Explicitly attempts to promote a restricted session to full access.
        """
        user_id = str(request.user.id)
        current_sid = request.auth.get("sid")
        ...
        try:
            ...
            return ResponseFactory.success(
                message="Successfully promoted to full access.",
                data={
                    "is_promoted": True,
                    "access": res["access"],
                    "refresh": res["refresh"],
...
class GeoLocationAPIView(APIView):
    ...
    def get(self, request):
        """
        Returns geolocation data for the current request's IP.
        """
        from authentication.sessions.infrastructure.cache import GeoLocationService

        context = build_auth_request_context(request)
        location = GeoLocationService.fetch_location(context.ip_address)
