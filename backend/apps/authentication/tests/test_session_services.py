...
class SessionServicesTests(TestCase):
    ...
    def test_persist_session_new_and_update(self):
        ...
        with patch("authentication.identity.infrastructure.cache.GraceJTIService.register_grace_jti") as grace_mock:
    ...
    def test_revoke_all_sessions(self):
        SessionManager.persist_session(
            user=self.user, session_id=str(uuid.uuid4()), access_jti="a1", refresh_jti="r1",
            fingerprint="f1", device_label="D1", device_entropy="e1", ip_address="1.1.1.1",
            expires_at=timezone.now() + timedelta(hours=1)
        )
        SessionManager.persist_session(
            user=self.user, session_id=str(uuid.uuid4()), access_jti="a2", refresh_jti="r2",
            fingerprint="f2", device_label="D2", device_entropy="e2", ip_address="2.2.2.2",
            expires_at=timezone.now() + timedelta(hours=1)
        )
        
        self.assertEqual(AuthSession.objects.filter(user=self.user, is_active=True).count(), 2)
        
        with patch("authentication.identity.infrastructure.cache.RedisSessionStore.remove_session"):
            count = SessionManager.revoke_all_sessions(str(self.user.id))
            self.assertEqual(count, 2)
            self.assertEqual(AuthSession.objects.filter(user=self.user, is_active=True).count(), 0)

    def test_anomaly_detection_impossible_travel(self):
        # Create a session in New York
        AuthSession.objects.create(
            user=self.user, session_id=uuid.uuid4(), access_jti="a1", refresh_jti="r1",
            fingerprint="f1", device_label="D1", device_entropy="e1", ip_address="1.1.1.1",
            expires_at=timezone.now() + timedelta(hours=1),
        ...
        with patch("authentication.sessions.application.services.GeoLocationService.calculate_distance", return_value=5585):
    ...
    def test_session_query_active_check(self):
        session_id = str(uuid.uuid4())
        SessionManager.persist_session(
            user=self.user, session_id=session_id, access_jti="a1", refresh_jti="r1",
            fingerprint="f1", device_label="D1", device_entropy="e1", ip_address="1.1.1.1",
            expires_at=timezone.now() + timedelta(hours=1)
        )
