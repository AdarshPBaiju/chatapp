...
import { AuthPhase, IdentityChallengePayload } from "./types";
...
export const useIdentityMachine = create<IdentityMachineState>((set) => ({
  phase: "IDENTIFY",
  ...
  setChallenge: (payload) => 
    set({
      ...
      isLoading: false,
      error: null,
      phase: payload.challenge_type === "select" 
        ? "METHOD_SELECT" 
        : payload.challenge_type === "password" 
          ? "PASSWORD_CHECK" 
          : payload.challenge_type === "mfa"
            ? (payload.allowed_methods.includes("totp") ? "MFA_TOTP" : "MFA_EMAIL_OTP")
            : "METHOD_SELECT"
    }),
