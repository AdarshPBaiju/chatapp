import { LoginRequest, RestrictedAuthPayload } from "../api/types";
...
export async function runIdentityInit(email: string): Promise<void> {
  ...
  machine.startFlow(email);
  try {
    const data = await identityInit({ email });
    machine.setChallenge(data);
    useAuthStore.getState().setAnonymous();
  } catch (error: any) {
    machine.setError(getErrorMessage(error, "Failed to initialize identity flow."));
    throw error;
  }
}
...
export async function runSignUpFinalizeFlow(params: {
  ...
  const { signUpFinalize } = await import("@/modules/auth/api/authApi");
